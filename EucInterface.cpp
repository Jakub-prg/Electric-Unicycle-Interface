...
void Euc::setCallback(void (*eucLoopCallback)(float,float,float,float,float,float,float,bool)) {
...
void Euc::tick() {
  ...
  this->eucLoop(data.voltage,data.speed,data.tempMileage,data.current,data.temperature,data.mileage,data.battery,data.dataIsNew);
}
...
Euc::UsableData Euc::makeRawDataUsable(Euc::RawData eucRawData) {
  ...
  eucUsableData.voltage = (float)(((eucRawData.voltage[0] << 8) | (eucRawData.voltage[1]) << 4) + 160); // volts
  ...
  //eucUsableData.unknownData = (float)((uint32_t)eucRawData.unknownData[0] << 24 | (uint32_t)eucRawData.unknownData[1] << 16 | (uint32_t)eucRawData.unknownData[2] << 8 | (uint32_t)eucRawData.unknownData[3]); // kilometers
  eucUsableData.mileage = (float)(((uint32_t)eucRawData.mileage[0] << 24 | (uint32_t)eucRawData.mileage[1] << 16 | (uint32_t)eucRawData.mileage[2] << 8 | (uint32_t)eucRawData.mileage[3]) / 3.906); // kilometers
  
  // % of battery
  if (eucUsableData.voltage > 8360) {
      eucUsableData.battery = 100;
      }
      else if (eucUsableData.voltage > 7200) {
      eucUsableData.battery = (eucUsableData.voltage - 7000) / 13;
      }
      else if (eucUsableData.voltage > 6700) {
      eucUsableData.battery = (float)((eucUsableData.voltage - 6700) / 32.5);
      }
      else {
      eucUsableData.battery = 0;
      }

  // eucUsableData.battery = (float)(100 - ((84 - eucUsableData.voltage) * 5.6)); // % of battery
  eucUsableData.voltage = (float)eucUsableData.voltage / 100;
